...
void loop()
{
  if(cnfg&&logging)
  {
    if((millis()-snp)>ms)
    {
      //Serial.println("condition inside");
      sendToFirebase(sensorCmd(),true);
      snp=millis();
    }
  }
  firebaseRead();
...
void firebaseRead()
{
  if(wifi_state)
  {
    Firebase.getInt(fdata,"/"+device_id+"/rx");
    if(fdata.intData()==1)
    {
      Firebase.getString(fdata,"/"+device_id+"/rxd");
      Firebase.setInt(fdata,"/"+device_id+"/rx",0);
      Serial.println(fdata.stringData());///////////
      sendToFirebase(cmd(fdata.stringData()),false);
    }
  }
}

void sendToFirebase(String a,boolean rd)
{
  Firebase.getInt(fdata,"/"+device_id+"/tx");
  if(rd)
  {
    Firebase.setString(fdata,"/"+device_id+"/txd",a);
    Firebase.setInt(fdata,"/"+device_id+"/tx",2);
  }
  else if(fdata.intData()==0)
  {
    Firebase.setString(fdata,"/"+device_id+"/txd",a);
    Firebase.setInt(fdata,"/"+device_id+"/tx",1);
  }
}
//############################################################################
...
String sensorCmd()
{
  return "sensor check :"+String(random(300));
}
...
void programCheck()
{
  ...
  if(prog_mark!='D')
  {
    Serial.println("ERASING EEPROM TO DEFAULT SET");
    erase();
